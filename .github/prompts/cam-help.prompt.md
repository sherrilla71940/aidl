...
| `/cam-pull` | Capture untracked VS Code files into `local/` by default or `sync/` when requested |
...
If this is your first time using copilot-asset-manager, start with `/cam-pull` to capture your existing VS Code config. Use `cam pull sync` when you want the imported files to become part of your portable sync set, then commit the result. On a new machine, run `/cam-push` to restore everything.
