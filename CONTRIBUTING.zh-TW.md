...
Upstream `copilot-asset-manager` 保持乾淨：只有腳本、`.github/` workspace 資源和文件。CI 會拒絕任何包含 `sync/` 或 `local/` 變更的 PR。

## 把改進回饋給 upstream

如果你修了同步腳本的 bug、改善了 workspace agent 或更新了文件，可以發 PR 回去。關鍵是把貢獻分支跟你的個人設定分開：
...
- `scripts/sync.sh` 或 `scripts/sync.ps1` 的 bug 修復
...
- 開 PR 前 `shellcheck scripts/sync.sh` 必須在本地通過
- 修改 `sync.sh` 時必須同步更新 `scripts/sync.ps1`
