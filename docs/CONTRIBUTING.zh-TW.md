...
- README 必須維持在 150 行以內
- 不可包含 `sync/` 或 `local/` 的變更 — CI 會自動拒絕

## 翻譯對稱

所有在 repo 根目錄和 `docs/` 下的 `.md` 檔案都必須有 `.zh-TW.md` 對應版本。三層機制確保這點：

| 層級 | 功能 |
|------|------|
| **Instruction**（`.github/instructions/translation-check.instructions.md`） | 編輯 `.md` 檔案時，agents 會提醒缺少 zh-TW 對應版本 |
| **Hook**（`.github/hooks/translation-check.md`） | Copilot pre-commit hook 在 commit 前警告並提供翻譯 |
| **CI**（`.github/workflows/ci.yml` → `translation-parity`） | 如果 zh-TW 檔案缺失或過期，PR 會失敗 |

翻譯檔案時，在 Copilot Chat 中使用 `translate` skill，或（即將推出）在終端機使用 `cam translate <file>`。
