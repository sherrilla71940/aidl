...
- Explain the difference between `user-sync/` and `user-local/` and when to use each
...
## user-sync/ vs user-local/

**user-sync/** — Personal assets synced bidirectionally with your VSCode user config. Everything here is tracked in git AND linked into VSCode (prompts, skills, instructions). Agents in `user-sync/agents/` are discovered by VSCode directly from this folder; they are not copied.
