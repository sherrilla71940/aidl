...
- **Wrong file location** — prompts go in `user-sync/prompts/`, skills in `user-sync/skills/<name>/`, agents in `user-sync/agents/`, instructions in `user-sync/instructions/`. Files outside `user-sync/` are not synced to VSCode.
...
- [ ] File is in the correct `user-sync/` subdirectory
